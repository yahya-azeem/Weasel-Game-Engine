...
Surface Surface::FromFile(const std::string& name)
{
...

	{
		...
		if (bitmap.GetLastStatus() != Gdiplus::Status::Ok)
		{
			...
			throw SException(__LINE__, __FILE__, ss.str());
		}
...
// surface SException stuff
Surface::SException::SException(int line, const char* file, std::string note) noexcept
	:
	Exception(line, file),
	note(std::move(note))
{}

const char* Surface::SException::what() const noexcept
{
...
const char* Surface::SException::GetType() const noexcept
{
	return " Graphics SException";
}

const std::string& Surface::SException::GetNote() const noexcept
{
	return note;
}